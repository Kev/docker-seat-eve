...
cat /seat/.env.clean | grep -v DB_HOST | grep -v DB_DATABASE | grep -v DB_USERNAME | grep -v DB_PASSWORD | grep -v CACHE_DRIVER | grep -v SESSION_DRIVER | grep -v QUEUE_DRIVER | grep -v DB_DEBUG> /seat/.env

echo "DB_HOST=mysql" >> /seat/.env
